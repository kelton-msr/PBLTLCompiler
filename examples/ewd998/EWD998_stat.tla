...
(***************************************************************************)
EXTENDS Integers, FiniteSets, Functions, SequencesExt, Randomization, CSV
...
vars == <<active, color, counter, pending, token, hasEmittedCSV, c>>
...
InitiateProbe ==
  ...
  /\ UNCHANGED <<active, counter, pending, hasEmittedCSV, c>>  
PassToken(i) ==
  (* Rules 2 + 4 + 7 *)
...
SendMsg(i) ==
  ...
  /\ UNCHANGED <<active, color, token, hasEmittedCSV, c>>
...
RecvMsg(i) ==
  ...
  /\ UNCHANGED <<c>>
  \*/\ UNCHANGED <<token, hasEmittedCSV, c>>>>                           

Deactivate(i) ==
  /\ active[i]
  /\ active' = [active EXCEPT ![i] = FALSE]
  /\ UNCHANGED <<color, counter, pending, token, hasEmittedCSV, c>>
...
Next ==
  /\ (System \/ Environment) 
  /\ hasEmittedCSV' = ~(((c["temp"]["counter"] <= 1) \/ c["temp"]["condition"]))
  /\ (IF (~(((c["temp"]["counter"] <= 1) \/ c["temp"]["condition"])) /\ ~(hasEmittedCSV)) THEN CSVWrite("%1$s", <<TRUE>>, "statPropsRes.csv") ELSE TRUE)
  ...
  /\ (IF Termination THEN CSVWrite("%1$s", <<c'["temp"]["hasAntecedent"]>>, "statPropsRes.csv") ELSE TRUE)
